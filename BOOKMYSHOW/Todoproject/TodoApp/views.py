...
def Update(req,id):
    ...
    if req.method=="POST": 
        ...
        movies.image.save(imagename,image,save=True)
        movies.banner.save(bannername,banner,save=True)
        description=req.POST.get('description','')        
        Movie.objects.filter(id=id).update(name=name,rate=rate,screen=screen,language=language,duration=duration,category=category,certification=certification,date=date,description=description)
        return redirect('home')
